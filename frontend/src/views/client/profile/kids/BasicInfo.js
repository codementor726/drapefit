...
import { addNewKid, kEditBasicInfo, kGetBasicInfo } from 'actions/client/kids';
...
const BasicInfo = () => {
  ...
  const [kid_count, setKid_count] = useState(1);

  useEffect(() => {
    localStorage.setItem('fitFor', 3);
    const order = localStorage.getItem('order');
    if (order === null && user.fitFor === 3) {
      setKid_count(1);
      dispatch(addNewKid(kid_count, navigate));
    } else {
      setKid_count(parseInt(order));
    }
  }, []);
  ...
  let newBasicInfo = {
    ...
    size_prefer_wear:
      kBasicInfo?.size_prefer_wear === 0
        ...
        ? 'Top Size'
        : 'Shoe Size',
    kids_first_name: kBasicInfo?.kids_first_name ?? '',
    ...
    <>
      ...
      <Formik
        ...
        onSubmit={async (values) => {
          if (saveReturn) {
            dispatch(kEditBasicInfo(values, order));
            saveReturn = false;
          } else {
            dispatch(kEditBasicInfo(values, order, navigate));
          }
        }}
      >
                  ...
                  <Grid container columnSpacing={4} padding="20px">
                    ...
                    <Grid item xs={12} sm={6}>
                      <Grid container>
                        ...
                        <Grid item xs={12}>
                          ...
                          /> */}
                          <FormControl fullWidth error={Boolean(touched.kids_first_name && errors.kids_first_name)}>
                            <InputLabel>
                              kids name. <span style={{ color: 'red' }}>*</span>
                            </InputLabel>
                            <OutlinedInput
                              size="small"
                              label="lbs. *"
                              autoFocus={Boolean(touched.kids_first_name && errors.kids_first_name)}
                              value={values.kids_first_name}
                              type="text"
                              name="kids_first_name"
                              onBlur={handleBlur}
                              onChange={handleChange}
                            />
                            {touched.kids_first_name && errors.kids_first_name && (
                              <FormHelperText id="helper-text-kids_first_name" error>
                                {errors.kids_first_name}
                              </FormHelperText>
                            )}
                          </FormControl>
                        </Grid>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Grid container>
                        <Grid item xs={12}>
                          <Typography className="basic-info-title">
                            Your Child Birthday? <span style={{ color: 'red' }}>*</span>
                          </Typography>
                        </Grid>
                        <Grid item xs={12}>
                    ...
                    <Grid item xs={12} sm={6}>
                      <Grid container>
                        <Grid item xs={12}>
                          <Typography className="basic-info-title">
                            Relationship with your child? <span style={{ color: 'red' }}>*</span>
                          </Typography>
                        </Grid>
                        <Grid item xs={12}>
                          <RadioButtonGroup

...
const FIT = () => {
  ...
  const initVal = { how_often_would_you_lik_fixes: '', applay_dt: new Date() };
  const schedules = [
        ...
        <Grid item xs={12} className="schedule">
          ...
          <Formik
            ...
            validationSchema={Yup.object().shape({
              how_often_would_you_lik_fixes: Yup.number().min(0).max(4).required('Please select FIT Schedule')
            })}
          ...
          >
            {({ errors, handleSubmit, setFieldValue, isSubmitting, touched, values }) => (
              <Form noValidate onSubmit={handleSubmit}>
                <FormControl
                  fullWidth
                  error={Boolean(touched.how_often_would_you_lik_fixes && errors.how_often_would_you_lik_fixes)}
                >
                  <RadioGroup
                    row
                    aria-labelledby="demo-customized-radios"
                    name="try_new_items_with_scheduled_fixes"
                    value={values.how_often_would_you_lik_fixes}
                  >
                    <Grid container spacing={2} sx={{ padding: { xs: '20px 10px', xl: '20px 8vw' } }}>
                      <Grid item xs={12} className="h-align-center">
                        {touched.how_often_would_you_lik_fixes && errors.how_often_would_you_lik_fixes && (
                          <FormHelperText id="helper-text-how-often-would-you-lik-fixes" error>
                            {errors.how_often_would_you_lik_fixes}
                          </FormHelperText>
                        )}
                      </Grid>
                      {schedules.map((item, index) => (
                        <Grid key={index} item xs={12} lg={4} sx={{ display: 'flex', justifyContent: 'center' }}>
                          <Box
                            ...
                            onClick={() => setFieldValue('how_often_would_you_lik_fixes', index)}
                            style={{
                              backgroundColor: values.how_often_would_you_lik_fixes === index ? '#e1e1e1' : '#fff'
                            }}
                          >
                            <Box width="100%">

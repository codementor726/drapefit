...
const editMenCustomDsgnBrand = asyncHandler(async (req, res) => {
  try {
    ...
    let { ...values } = req.body;
    let { is_progressbar } = await UserDetail.findOne({ where: { user_id: user_id } });
    let menbrand = MenBrand.findOne({
      where: { user_id: values.user_id }
    });
    let menstats = MenStats.findOne({
      where: { user_id: values.user_id }
    });
    let menstyle = MenStyle.findOne({
      where: { user_id: values.user_id }
    });
    if (menbrand) {
      await MenBrand.update({ ...values }, { where: { user_id: values.user_id } });
    } else {
      await MenBrand.create({ user_id: values.user_id });
      await MenBrand.update({ ...values }, { where: { user_id: values.user_id } });
    }
    if (menstats) {
      await MenStats.update({ ...values }, { where: { user_id: values.user_id } });
    } else {
      await MenStats.create({ user_id: values.user_id });
      await MenStats.update({ ...values }, { where: { user_id: values.user_id } });
    }
    if (menstyle) {
      await MenStyle.update({ ...values }, { where: { user_id: values.user_id } });
    } else {
      await MenStyle.create({ user_id: values.user_id });
      await MenStyle.update({ ...values }, { where: { user_id: values.user_id } });
    }
    if (is_progressbar < 100) {
      await UserDetail.update({ is_progressbar: 100 }, { where: { user_id: values.user_id } });
    }

...
const editWomenFPCustomDsgnBrand = asyncHandler(async (req, res) => {
  try {
    ...
    if (womenBrand) {
      await WomenShoePrefer.update({ ...values }, { where: { user_id: user_id } });
    } else {
      await WomenShoePrefer.create({
        user_id: user_id
      });
      await WomenShoePrefer.update({ ...values }, { where: { user_id: user_id } });
    }
    ...
    if (womenProfileNote) {
      await WomenStyle.update({ profile_note: req.body.profile_note }, { where: { user_id: user_id } });
    } else {
      await WomenStyle.create({
        user_id: user_id
      });
      await WomenStyle.update({ ...values }, { where: { user_id: user_id } });
    }

    if (womenImgUrl) {
      await CustomDesign.update({ ...values }, { where: { user_id: user_id } });
    } else {

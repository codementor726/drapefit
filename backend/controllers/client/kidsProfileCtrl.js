...
const editKidBasicInfo = asyncHandler(async (req, res) => {
  try {
    console.log('--------------------');
    console.log(parseInt(req.params.order));
...
const editKidStyleFit = asyncHandler(async (req, res) => {
  try {
    ...
    console.log('------------------------------------------');
    console.log(kid_id);
    await KidsDetail.update({ ...rest, is_progressbar: 50 }, { where: { user_id: user_id, id: kid_id } });
    await kidClothingType.update({ ...rest }, { where: { user_id: user_id, kid_id: kid_id } });
    await KidSizeFit.update({ ...rest }, { where: { kid_id: kid_id } });
    //-- okay
...
const getKidStyleFit = asyncHandler(async (req, res) => {
  try {
    ...
    kidsDetail = await KidsDetail.findOne({
      where: { user_id: user_id, kid_count: kid_count }
    });
    if (!kidsDetail) {
      await KidsDetail.create({
        user_id: user_id,
        kid_count: kid_count,
        kids_frequency_arts_and_crafts: 'Sometimes',
        kids_frequency_theatre: 'Sometimes',
        kids_frequency_biking: 'Sometimes',
        kids_frequency_dance: 'Sometimes',
        kids_frequency_playing_outside: 'Sometimes',
        kids_frequency_musical_instruments: 'Sometimes',
        kids_frequency_musical_instruments: 'Sometimes',
        kids_frequency_reading: 'Sometimes',
        kids_frequency_video_games: 'Sometimes'
      });
      kidsDetail = await KidsDetail.findOne({
        where: { user_id: user_id, kid_count: kid_count }
      });
    ...
    if (!kidFit) {
      await KidSizeFit.create({
        user_id: user_id,
        kid_id: kid_id,
        shirt_sleeve_length: 'None',
        kids_fit_challenge_shirt_torso_length: 'None',
        kids_fit_challenge_shirt_torso_width: 'None',
        kids_fit_challenge_pant_waist: 'None',
        kids_fit_challenge_pant_leg_length: 'None',
        kids_fit_challenge_pant_leg_width: 'None',
        t_shirts: 'Sometimes',
        sweats_shirts: 'Sometimes',
        button_downs: 'Sometimes',
        polo_shirts: 'Sometimes',
        shorts: 'Sometimes',
        jacket_coats: 'Sometimes',
        jeans: 'Sometimes',
        trousers_chino: 'Sometimes',
        sweatspaint: 'Sometimes',
        shoes: 'Sometimes',
        pajamas: 'Sometimes',
        sweaters: 'Sometimes',
        top_blouses: 'Sometimes',
        dreses_rompers: 'Sometimes',
        leggings: 'Sometimes',
